...
import com.architect.coders.mu8.mock.mockedComicsResponse
...
class ComicsUiTest {
    ...
    @get:Rule
    val activityTestRule = ActivityTestRule(ComicsActivity::class.java, false, false)
    ...
    fun setUp() {
        ...
        val intent = Intent(instrumentation.targetContext, ComicsActivity::class.java)
    ...
    fun tapOnComicNavigatesToDetail() {
        // Arrange
        mockWebServer.enqueue(MockResponse().setResponseCode(200).setBody(mockedComicsResponse))
        SystemClock.sleep(1000)

        // Act
        onView(withId(R.id.comic_activity_recycler)).perform(actionOnItemAtPosition<ViewHolder>(1, ViewActions.click()))

        // Assert
        onView(withText("Marvel Previews (2017)"))
